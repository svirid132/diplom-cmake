...
QString HandlerDocument::handleMathRaplace(const QString &specSymb, DocxFlags::Texts specText) {
    ...
    if (docData.N0 != -1) {//Не тествовый режим!
        ...
        text
            ...
            .replace(specSymbol.posDescWritter, docData.docCommission.posDescWritter)
            .replace(specSymbol.nameWritter, docData.docCommission.nameWritter)
            .replace(specSymbol.product, docData.product)
            .replace(specSymbol.dateProduct, docData.dateProduct)
            .replace(specSymbol.posDescMain, docData.docCommission.posDescMain)
            .replace(specSymbol.posDescMembOne, docData.docCommission.posDescMembOne)
            .replace(specSymbol.posDescMembTwo, docData.docCommission.posDescMembTwo)
            .replace(specSymbol.rudnik, docData.rudnik)
            .replace(specSymbol.koefZap, QString::number(docData.koefZap))
            .replace(specSymbol.nameMain, docData.docCommission.nameMain)
            .replace(specSymbol.nameMembOne, docData.docCommission.nameMembOne)
            .replace(specSymbol.nameMembTwo, docData.docCommission.nameMembTwo)
